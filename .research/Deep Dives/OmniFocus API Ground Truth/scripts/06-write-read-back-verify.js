...
(() => {
  ...
  r += `  p.task.status():  ${matchTaskStatus(t.status())} (raw type: ${typeof t.status()})\n`;
  ...
  r += `  reviewInterval: ${ri ? `steps=${ri.steps()}, unit=${ri.unit()}` : "null"}\n`;
  try {
    r += `  nextTask: ${p.nextTask() ? p.nextTask().name() : "null"}\n`;
  } catch(e) {
    r += `  nextTask: ERROR (${e.message})\n`;
  }
  r += `  folder: ${p.folder() ? p.folder().name() : "null"}\n`;
  ...
  for (let i = 0; i < childTasks.length; i++) {
    ...
    r += `  status: ${matchTaskStatus(ct.status())}\n`;
    ...
    try {
      r += `  parent: ${ct.parentTask() ? ct.parentTask().name() : "null"}\n`;
    } catch(e) {
