...
(() => {
  ...
  r += `Task.status() types (first ${taskSample}): ${JSON.stringify(taskStatuses)}\n`;
  ...
  // Helper: test a status value against known constants
  ...
  let taskStatusDist = {};
  for (let i = 0; i < taskSample; i++) {
    const label = matchTaskStatus(tasks[i].status());
  ...
  // Are the enum values the same across types?
  try {
