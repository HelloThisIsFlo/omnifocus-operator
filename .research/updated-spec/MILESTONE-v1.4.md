...
Concurrent batch calls from separate agent threads are not serialized at the server level. Items within a single batch are serial; interleaving across batches is possible. Document as a known limitation — serial execution guarantee is a ~~v1.7~~ v1.8 concern.
