# Requirements Archive: v1.3 Read Tools
...
- [x] **DESC-07**: All `@mcp.tool()` function docstrings in `server.py` use constants from `descriptions.py` — no inline tool description strings. Enforcement test catches new tools with inline docstrings.
- [x] **DESC-08**: All tool descriptions are at most 2048 bytes — Claude Code truncates at 2KB. Enforcement test checks each tool description constant's UTF-8 byte length and fails with the exact byte count if exceeded.
...
- **OUTPUT-02**: Agent can get TaskPaper format output (~~v1.4.2~~ — see MAYBE-IDEAS.md, slot reclaimed for hotfix)
...
| DESC-07 | Phase 36.3 | Complete |
| DESC-08 | Phase 36.3 | Complete |
