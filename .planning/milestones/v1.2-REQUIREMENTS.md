# Requirements Archive: v1.2 Writes & Lookups
...
### ~~Output, UI & Remaining Tools (v1.4)~~ → ~~Split into v1.4/v1.4.1/v1.4.2/v1.4.3/v1.5~~ (v1.4.2 reclaimed for hotfix)

- **OUTP-01**: Perspectives support → ~~v1.4~~ → v1.5
- **OUTP-02**: Field selection → v1.4
- **OUTP-03**: TaskPaper output format → ~~v1.4~~ → ~~v1.4.2~~ (see MAYBE-IDEAS.md)
- **OUTP-04**: Project writes (add_projects, edit_projects) → ~~v1.4~~ → v1.4.3
- **OUTP-05**: delete_tasks → v1.4

### ~~Production Hardening (v1.5)~~ → v1.6

- **HARD-01**: Retry logic for bridge timeouts → ~~v1.5~~ → v1.6
- **HARD-02**: Crash recovery → ~~v1.5~~ → v1.6
- **HARD-03**: Fuzzy search → ~~v1.5~~ → v1.4.1
- **HARD-04**: App Nap mitigation → ~~v1.5~~ → v1.6
...
| delete_tasks | ~~Deferred to v1.4~~ → v1.4; `drop` + `edit_tasks` with movement handles most cases |
| Project writes | ~~Unverified OmniJS APIs~~ → v1.4.3; low priority for current workflow |
| Tag/folder writes | Low priority; tags created manually |
| Idempotency / retry logic | Production hardening (~~v1.5~~ → v1.6); v1.2 proves the pipeline |
