...
- [ ] **Phase 29: Dependency Swap & Imports** - Foundation: switch to fastmcp>=3, update imports, add progress reporting, update docs
...
**Success Criteria** (what must be TRUE):
  1. `uv run pytest` passes with `fastmcp>=3.1.1` as the sole MCP dependency
  2. All imports reference `fastmcp` -- zero remaining `mcp.server.fastmcp` imports in src/
  3. Tool handlers use `ctx.lifespan_context` shorthand (not `ctx.request_context.lifespan_context`)
...
| 29. Dependency Swap & Imports | v1.2.2 | 1/2 | In progress | - |
